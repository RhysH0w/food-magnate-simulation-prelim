...
class Simulation:
    ...
    def __ProcessAddRemoveHouseholdsEvent(self):
        HouseRandNumb = random.random()
        NoOfHouseholdsChange = random.randint(1, 4)
        if HouseRandNumb < 0.3:
            for i in range(NoOfHouseholdsChange):
                settlement = Settlement()
                settlement._Households.pop(random.randrange(len(settlement._Households)))
            print(str(NoOfHouseholdsChange) + " households have been removed from the settlement.")
        else:
            for Count in range(1, NoOfHouseholdsChange + 1):
                self._SimulationSettlement.AddHousehold()
            print(str(NoOfHouseholdsChange) + " new households have been added to the settlement.")
